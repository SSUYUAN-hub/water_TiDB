...
function calculateSalaryFulltime($startTime, $endTime, float $hourlyRate, bool $hasBreak = false): array {
    ...
    // 加班費計算（依勞基法）
    // 門檻：工時 > 8h5min 才觸發加班
    // 計算基準：超過 8h 整的部分（與勞動部一致）
    $OT_TRIGGER = 8 + 5 / 60; // 8h5min：觸發門檻
    $normalHours = 0;
    $overtime1   = 0; // 前 2 小時加班（×4/3）
    $overtime2   = 0; // 超過 2 小時後（×5/3）

    if ($actualWorkHours <= $OT_TRIGGER) {
        // 未超過門檻，全部算正常工時
        $normalHours = $actualWorkHours;
    } else {
        // 超過門檻：計算基準從 8h 整開始算加班
        $normalHours = 8.0;
        $remaining   = $actualWorkHours - 8.0;
        if ($remaining <= 2) {
            $overtime1 = $remaining;
        } else {
            $overtime1 = 2;
    ...
    return [
        ...
        'overtime_pay'   => (int)ceil($overtimePay),
        'salary'         => (int)ceil($salary),
